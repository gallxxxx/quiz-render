...
// «викторина10» — конвейер второго формата. Его тут не было вовсе:
// список писался, когда на сборку уезжала только «Это или то», и
// переезд молча увёз бы репозиторий без половины проекта.
const ПАПКИ = ["src", "tools", ".github", "public", "викторина10"];
const ФАЙЛЫ = [
  "package.json",
  "package-lock.json",
  "remotion.config.ts",
...
const НЕ_НАДО = (относительный) => {
  ...
  // Данные выпусков: расшифровки, переводы, выбранные человеком
  // фотографии, архивы папок. Это НЕ код, и наружу им нельзя.
  if (/^(задания|задания10|ответы10|выпуски10|состояния10|звук10)(\/|$)/.test(п)) return true;
  if (/(^|\/)(out|__pycache__)(\/|$)/.test(п)) return true;
  if (/^викторина10\/(картинки|фон)\//.test(п)) return true;   // свои снимки и клипы
...
// Сломанный сценарий наружу не выпускаем. GitHub на кириллицу в именах
// работ и шагов отвечает «workflow file issue» и не запускает ничего —
// ни журнала, ни шагов, причину искать негде. Дешевле поймать здесь.
{
  const проверка = spawnSync(process.execPath,
    [path.join(HERE, "проверить-сценарии.mjs")], { stdio: "inherit" });
  if (проверка.status !== 0) {
    console.log("");
    console.log("Ничего не переношу, пока это не исправлено.");
    process.exit(1);
  }
}
